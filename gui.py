...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        ...
        for pair in self.agent.pairs:
            if state.matrix[pair.cell1.line][pair.cell1.column] == constants.PRODUCT:
                if state.matrix[pair.cell1.line][pair.cell1.column-1] == constants.EMPTY:
                    state.column_forklift = pair.cell1.column - 1
                else:
                    state.column_forklift = pair.cell1.column + 1
            else:
            ...
            if state.matrix[pair.cell2.line][pair.cell2.column] == constants.PRODUCT:
                if state.matrix[pair.cell2.line][pair.cell2.column-1] == constants.EMPTY:
                    goal_position = Cell(pair.cell2.line, pair.cell2.column - 1)
                else:
                    goal_position = Cell(pair.cell2.line, pair.cell2.column + 1)
            else:
                goal_position = Cell(pair.cell2.line, pair.cell2.column)

            problem = WarehouseProblemSearch(state, goal_position)
            sol = self.agent.solve_problem(problem)
            pair.value = int(sol.cost)
            pair.path = sol.path
            pair.path.insert(0, Cell(state.line_forklift, state.column_forklift))
...
class SolutionRunner(threading.Thread):
    ...
    def run(self):
        self.thread_running = True
        forklift_path, steps, goal_cells = self.best_in_run.obtain_all_path()
        ...
        for step in range(steps - 1):
            ...
            for j in range(len(forklift_path)):
                ...
                if step < len(forklift_path[j]) - 1:
                    ...
                    if len(goal_cells[j]) > 0 and self.state.matrix.shape[1] != new_cell.column + 1:
                        if goal_cells[j][0].line == new_cell.line and goal_cells[j][0].column == new_cell.column + 1:
                            self.state.matrix[new_cell.line][new_cell.column+1] = constants.PRODUCT_CATCH
                            del goal_cells[j][0]
                    if self.state.matrix.shape != new_cell.column - 1:
                        if len(goal_cells[j]) > 0 and  goal_cells[j][0].line == new_cell.line and goal_cells[j][0].column == new_cell.column - 1:
                            self.state.matrix[new_cell.line][new_cell.column - 1] = constants.PRODUCT_CATCH
                            del goal_cells[j][0]
                            if len(goal_cells[j]) > 0 and  goal_cells[j][0].line == new_cell.line and goal_cells[j][0].column == new_cell.column + 1:
                                self.state.matrix[new_cell.line][new_cell.column + 1] = constants.PRODUCT_CATCH
                    ...
                    self.state.matrix[old_cell[j].line][old_cell[j].column] = constants.EXIT
